...
const sendEmail = async (options) => {
  try {
    ...
    const mailOptions = {
      from: `${process.env.FROM_NAME || 'Sports Ticketing'} <${process.env.EMAIL_USER}>`,
      to: options.email,
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
          ...
          <p><strong>Seat:</strong> ${ticket.seatInfo.category} - ${ticket.seatInfo.seatNumber}</p>
  ...
  gameReminder: (ticket, event, hoursUntil) => ({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>Game Reminder</h2>
        <p>Hi ${ticket.user.name},</p>
        <p>This is a reminder that <strong>${event.title}</strong> starts in ${hoursUntil} hours.</p>
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
          <h3>Event Details:</h3>
          <p><strong>Date:</strong> ${new Date(event.date).toLocaleDateString()}</p>
          <p><strong>Time:</strong> ${new Date(event.date).toLocaleTimeString()}</p>
          <p><strong>Venue:</strong> ${event.venue.name}</p>
          <p><strong>Your Seat:</strong> ${ticket.seatInfo.category} - ${ticket.seatInfo.seatNumber}</p>
        </div>
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f8f9fa; padding: 20px; border-radius: 5px; margin: 20px 0;">
          ...
          <p><strong>Valid From:</strong> ${new Date(seasonPass.validityPeriod.start).toLocaleDateString()}</p>
          <p><strong>Valid Until:</strong> ${new Date(seasonPass.validityPeriod.end).toLocaleDateString()}</p>
